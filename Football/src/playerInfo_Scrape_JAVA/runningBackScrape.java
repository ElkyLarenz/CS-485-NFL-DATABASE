...
public class runningBackScrape {
		...
		try {
			...
			for (Element row : doc.select("table.data-table1  tr")) {
				...
				if (row.select("td:nth-of-type(2)").text().equals("")) {
					...
					final String playerAtt = row.select("td:nth-of-type(5)").text();
					final String playerAttG = row.select("td:nth-of-type(6)").text();
					final String playerYds = row.select("td:nth-of-type(7)").text();
					final String playerAvg = row.select("td:nth-of-type(8)").text();
					final String playerYdsG = row.select("td:nth-of-type(9)").text();
					final String playerTd = row.select("td:nth-of-type(10)").text();
					final String playerLng = row.select("td:nth-of-type(11)").text();
					final String player1st = row.select("td:nth-of-type(12)").text();
					final String player1stPerc = row.select("td:nth-of-type(13)").text();
					final String player20Plus = row.select("td:nth-of-type(14)").text();
					final String player40Plus = row.select("td:nth-of-type(15)").text();
					final String playerFum = row.select("td:nth-of-type(16)").text();
					send(playerRank, playerName, playerTeam, playerPos, playerAtt, playerLng, playerAttG, playerYds, playerAvg, playerYdsG,
	...
	public static void send(String ra, String na, String te, String pos, String att, String attG, String yds, String avg,
